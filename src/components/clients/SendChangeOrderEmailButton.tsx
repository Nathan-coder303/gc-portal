...
export default function SendChangeOrderEmailButton({
  changeOrderId,
  clientId,
  companyId,
  orderTitle,
  ...
  const defaultBody = `Dear ${firstName},\n\nPlease find attached the change order for your project.\n\nDo not hesitate to contact us with any questions.\n\n${MIKE_SIGNATURE}`;

  const orderLabel = orderNumber ? `Change Order ${orderNumber}` : "Change Order";
  const defaultSubject = `${orderLabel} – ${orderTitle}`;
  ...
  async function send() {
    ...
    setResult(null);
    try {
      const res = await fetch(`/api/${companyId}/send-change-order-email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ changeOrderId, clientId, to, cc: cc.trim() || undefined, bcc: bcc.trim() || undefined, subject, body, coverType, page2 }),
      });
      ...
      if (res.ok) {
        ...
        console.error("send-change-order-email error:", res.status, text.slice(0, 500));
      }
    } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="w-full max-w-lg rounded-2xl p-6 space-y-4" style={{ background: "#161b22", border: "1px solid #30373f" }}>
            <div className="flex items-center justify-between">
              <h2 className="text-base font-bold" style={{ color: "#e6edf3" }}>Send Change Order via Gmail</h2>
            ...
            <div className="flex items-center gap-3">
              <p className="text-xs flex-1" style={{ color: "#8b949e" }}>
                Sending: <span style={{ color: "#C9A84C" }}>{orderLabel} – {orderTitle}</span>
              </p>
              <button

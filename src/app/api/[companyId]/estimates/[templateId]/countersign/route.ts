...
export async function POST(
  ...
  if (!signatureData) return NextResponse.json({ error: "Missing signature" }, { status: 400 });
  ...
  if (!template.signedAt) return NextResponse.json({ error: "Client has not signed yet" }, { status: 400 });
  ...
  const divisions = template.divisions.map((d) => ({
    ...
    groups: d.groups.map((g) => ({
      ...
      items: g.items.map((i) => ({
        ...
        visibleInPdf: i.visibleInPdf, notes: i.notes,
      })),
    })),
    items: d.items.map((i) => ({
      id: i.id, name: i.name, detail: i.detail, unit: i.unit,
  ...
  try {
    ...
    const bodyText = [
      ...
      `Client: ${clientLabel}`,
      `Client signed: ${template.signedAt?.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}`,

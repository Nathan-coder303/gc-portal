...
// POST /api/admin/resend-countersign
// Body: { templateId: string, secret: string }
export async function POST(req: NextRequest) {
  ...
  if (!template.signedAt) return NextResponse.json({ error: "Not signed by client" }, { status: 400 });
  if (!template.counterSignedAt) return NextResponse.json({ error: "Not countersigned" }, { status: 400 });
  if (!template.counterSignatureData) return NextResponse.json({ error: "No contractor signature data" }, { status: 400 });
  ...
  const buffer = await renderTemplatePdf({
    ...
    contractorSignatureData: template.counterSignatureData,
    contractorSignedAt: template.counterSignedAt,
  ...
  const bodyText = [
    ...
    `Contractor signed: ${template.counterSignedAt?.toLocaleString("en-US", { month: "long", day: "numeric", year: "numeric" })}`,
    ``,
  ...
  return NextResponse.json({ success: true, sentTo: sent, filename });
}

...
export default function SignDocPage() {
  ...
  useEffect(() => {
    fetch(`/api/sign-doc?token=${token}`)
      .then(r => r.json())
      ...
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "#f8fafc" }}>
        <div className="max-w-md w-full rounded-2xl p-8 text-center shadow-lg" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
          ...
          <p className="text-xs mt-4" style={{ color: "#94a3b8" }}>
            Thank you. {info.companyName} will countersign and send you a fully executed copy.
          </p>
          <a
            href={info.pdfUrl} target="_blank" rel="noopener noreferrer"
    ...
    <div className="min-h-screen py-10 px-4" style={{ background: "#f8fafc" }}>
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
          ...
          {info.description && <p className="text-sm mt-1" style={{ color: "#64748b" }}>{info.description}</p>}
          {info.clientName && <p className="text-sm mt-1" style={{ color: "#64748b" }}>{info.clientName}</p>}
        ...
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: "2px solid #e2e8f0", background: "#ffffff", touchAction: "none" }}>
            <canvas
              ref={canvasRef} width={600} height={180} className="w-full"
        ...
        <div className="rounded-2xl p-6 shadow-sm space-y-4" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
          <div>
            ...
            <input
              ...
              placeholder="Your full name" className="w-full rounded-xl px-4 py-2.5 text-sm"
          ...
          <button
            onClick={handleSubmit} disabled={submitting || !hasDrawn || !name.trim()}
          ...
          <p className="text-xs text-center" style={{ color: "#94a3b8" }}>
            By signing, you confirm you have reviewed and agree to the document above.
          </p>
        </div>

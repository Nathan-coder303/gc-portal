...
export default async function HubPage({
  ...
  if (tab === "memory") {
    const company = await prisma.company.findFirst({
      ...
      select: { requestTrackerUrl: true, requestTrackerDate: true, backupFileUrl: true, backupFileDate: true },
    });
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-8 space-y-6">
        ...
        <div className="rounded-2xl p-5" style={{ background: "#1e2736", border: `1px solid ${company?.requestTrackerUrl ? "#C9A84C44" : "#30373f"}` }}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="font-semibold mb-1" style={{ color: "#e6edf3" }}>Full Conversation Tracker</div>
                {company?.requestTrackerUrl ? (
                  <p className="text-sm" style={{ color: "#C9A84C" }}>As of {company.requestTrackerDate ?? "unknown date"}</p>
                ) : (
                  <p className="text-sm" style={{ color: "#8b949e" }}>Not yet uploaded.</p>
                )}
                <p className="text-xs mt-1" style={{ color: "#484f58" }}>Excel · all Claude conversations · updated nightly at 2 AM</p>
              </div>
            </div>
            {company?.requestTrackerUrl && (
              <a href={`/api/${params.companyId}/download-request-tracker`} download="GC_Portal_Conversations.xlsx"
        ...
        {/* Database Backup */}
        <div className="rounded-2xl p-5" style={{ background: "#1e2736", border: `1px solid ${company?.backupFileUrl ? "#22c55e44" : "#30373f"}` }}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4">
              <span className="text-3xl">💾</span>
              <div>
                <div className="font-semibold mb-1" style={{ color: "#e6edf3" }}>Database Backup</div>
                {company?.backupFileUrl ? (
                  <p className="text-sm" style={{ color: "#22c55e" }}>As of {company.backupFileDate ?? "unknown date"}</p>
                ) : (
                  <p className="text-sm" style={{ color: "#8b949e" }}>Not yet uploaded.</p>
                )}
                <p className="text-xs mt-1" style={{ color: "#484f58" }}>SQL dump · full Neon database · updated nightly at 2 AM</p>
              </div>
            </div>
            {company?.backupFileUrl && (
              <a href={`/api/${params.companyId}/download-backup`} download="GC_Portal_DB_Backup.sql"
                className="shrink-0 px-4 py-2 rounded-lg text-sm font-semibold"

...
import PortalTabContainer from "@/components/portal/PortalTabContainer";
...
export default async function ClientPortalPage({ params }: { params: { clientId: string } }) {
  ...
  const tabs: TabKey[] = [];
  if (showMessages)    tabs.push("messages");
  if (showDailyPhotos) tabs.push("photos");
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0d1117", color: "#e6edf3" }}>
      ...
      {tabs.length === 0 ? (
        ...
        <PortalTabContainer tabs={tabs}>
          {{
            messages:  showMessages    ? <PortalMessages clientId={client.id} />    : undefined,
            photos:    showDailyPhotos ? <PortalPhotoGallery clientId={client.id} /> : undefined,
            logs:      showDailyLogs   ? <PortalDailyLogs clientId={client.id} />   : undefined,
            documents: showDocuments   ? documentsContent                            : undefined,
          }}
        </PortalTabContainer>
      )}
    </div>
